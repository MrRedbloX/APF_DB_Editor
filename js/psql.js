            ...
            client.connect(function(err,client) {
              if(err){
               ...
               res.status(400).send(err);
                ...
                console.log("Connection successful");
                client.query("SELECT conname, pg_catalog.pg_get_constraintdef(r.oid, true) as condef FROM pg_catalog.pg_constraint r WHERE r.conrelid = '"+req.query.table+"'::regclass AND r.contype = 'f';" , function(err,result) {
                  client.end(); // closing the connection;
                  if(err){
                     console.log(err);
                     res.status(400).send(err);
