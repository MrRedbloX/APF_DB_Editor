      ...
      if(currentTableSelected != null){
        ...
        for(let j=0; j<columnList.length; j++){
          ...
          if(elt.nodeName === "INPUT"){
            if(elt.value == null) valueList.push("NULL");
            else{
            ...
            if(elt.selectedIndex == null || elt.options[elt.selectedIndex] == null) valueList.push("NULL");
            else{
    ...
    if(confirm("Are you sure you want to save this record ?")){
      ...
      if(currentTableSelected != null){
        ...
        for(let j=0; j<columnList.length; j++){
          let elt = document.getElementById(columnList[j]);
          if(elt.nodeName === "INPUT"){
            if(elt.value == null || elt.options[elt.selectedIndex] == null) valueList.push("NULL");
            else{
            ...
            if(elt.selectedIndex == null) valueList.push("NULL");
            else{
      ...
      for(let i=0; i<postgresqlScope.valuesOfConstraint.length; i++){
        if(att === postgresqlScope.valuesOfConstraint[i].name){
          for(let j=0; j<postgresqlScope.valuesOfConstraint[i].values.length; j++){
            if(postgresqlScope.valuesOfConstraint[i].values[j] !== val.toString())
              $scope.references.push(postgresqlScope.valuesOfConstraint[i].values[j]);
          }
          break;
        }
      }
