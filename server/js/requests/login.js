var loginConString = "postgres://postgres:postgres@10.237.169.202:5432/postgres"; //The database ip

//The MD5 conversion function
...
module.exports = {
  //From a MD5 user+pass return or not an ID
    ...
    client.connect(function(err,client) {
      if(err){
       ...
       res.status(400).send(err);
        ...
        console.log("Connection successful");
        client.query("INSERT INTO apf_id (username,md5,mail,md5namemail,mail_validation) VALUES ('"+req.query.id+"','"+req.query.md5+"','"+req.query.mail+"','"+req.query.md5NameMail+"',"+req.query.validate+");" ,function(err,result) {
          client.end(); // closing the connection;
          if(err){
             console.log(err);
             res.status(400).send(err);
  ...
  //From a MD5 last_name+first_name+email return or not an ID
  ...
  //(In build, not working) Send an email confirmation for a user who wants to register
